...
| FR-DRIVE-9 | External items | `GET /api/drives/external` returns top-level external entry points (folders/files whose parent is `__shared__` — computer backups + shared folders). `GET /api/drives/:driveId/external-folders/:googleFolderId` drills in. |
