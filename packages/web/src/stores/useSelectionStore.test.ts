...
describe('useSelectionStore', () => {
  ...
  describe('isSameItem', () => {
    ...
    it('should correctly compare folder items with id', () => {
      const folder1 = {
        type: 'folder',
        item: { id: '1', googleFolderId: 'g1' } as DriveFolder,
      } as const;
      const folder2 = {
        type: 'folder',
        item: { id: '1', googleFolderId: 'g2' } as DriveFolder,
      } as const;
      const folder3 = {
        type: 'folder',
        item: { id: '2', googleFolderId: 'g1' } as DriveFolder,
      } as const;
  ...
  describe('store functions', () => {
    ...
    it('should toggle folder selection correctly (with and without id)', () => {
      ...
      useSelectionStore
        .getState()
        .toggleSelection({ type: 'folder', item: { ...folderWithId } as DriveFolder });
      expect(useSelectionStore.getState().selectedItems).toEqual([item2]);

      useSelectionStore.getState().toggleSelection({
        type: 'folder',
        item: { name: 'diff_name', googleFolderId: 'g2' } as DriveFolder,
      });
      expect(useSelectionStore.getState().selectedItems).toEqual([]);
    });
