        ...
        <tbody>
          {logs.map((log) => (
            <tr key={log.id} className="border-b last:border-0 hover:bg-slate-50">
              <td className="px-4 py-2 text-sm text-slate-500">{new Date(log.createdAt || (log as AuditLog & { created_at?: string; actor_email?: string; action_type?: string; resource_name?: string; resource_id?: string }).created_at || '').toLocaleString()}</td>
