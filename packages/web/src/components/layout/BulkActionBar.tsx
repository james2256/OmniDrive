...
export const BulkActionBar: React.FC<BulkActionBarProps> = ({
  ...
  const confirmDelete = async () => {
    ...
    addToast(
      'info',
      `${isTrashView ? 'Permanently deleting' : 'Deleting'} ${selectedItems.length} items...`,
    );
    ...
    if (failCount === 0) {
      addToast(
        'success',
        `${isTrashView ? 'Permanently deleted' : 'Deleted'} ${successCount} items`,
      );
    } else {
      ...
      <div className="fixed bottom-3 left-2 right-2 sm:left-1/2 sm:right-auto sm:-translate-x-1/2 z-50 flex flex-wrap items-center gap-1.5 sm:gap-2 bg-card/80 backdrop-blur-md border border-slate-200 text-slate-800 rounded-2xl sm:rounded-full shadow-2xl px-3 py-2 sm:px-4 sm:py-3 animate-in fade-in-0 slide-in-from-bottom-5 duration-300">
        <div className="flex items-center gap-2 sm:gap-3 sm:border-r sm:border-slate-200 sm:pr-4">
          <Button
            ...
            variant="ghost"
            className="p-1.5 sm:p-2 text-slate-500 rounded-full"
            aria-label="Clear selection"
          >
            <X size={18} />
          </Button>
          <span className="font-medium text-sm text-primary bg-primary/10 px-2.5 py-0.5 rounded-full">
          ...
          {isTrashView ? (
            <>
              <Button
                ...
                disabled={isProcessing}
                variant="ghost"
                className="px-2.5 sm:px-3 py-2 hover:bg-green-50 hover:text-green-600 rounded-full text-sm"
              ...
              <Button
                ...
                disabled={isProcessing}
                variant="ghostDanger"
                className="px-2.5 sm:px-3 py-2 rounded-full text-sm text-slate-600"
            ...
            <>
              ...
              <Button
                ...
                disabled={isProcessing}
                variant="ghost"
                className="px-2.5 sm:px-3 py-2 rounded-full text-sm"
