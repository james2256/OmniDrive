  ...
  return (
    ...
    <div className="fixed bottom-4 left-2 right-2 sm:left-1/2 sm:right-auto sm:-translate-x-1/2 z-50 flex flex-wrap items-center gap-2 bg-card/80 backdrop-blur-md border border-slate-200 text-slate-800 rounded-2xl sm:rounded-full shadow-2xl px-4 py-3 animate-in fade-in-0 slide-in-from-bottom-5 duration-300">
      ...
      <div className="flex flex-wrap items-center gap-1 sm:gap-2 sm:pl-2">
        {isTrashView ? (
          <>
            <button onClick={handleRestore} disabled={isProcessing} className="flex items-center gap-2 px-3 py-2 hover:bg-green-50 text-slate-600 hover:text-green-600 rounded-full transition-colors text-sm font-medium" title="Restore selected items">
              <RotateCcw size={16} /> <span>Restore</span>
            </button>
            <button onClick={handleDelete} disabled={isProcessing} className="flex items-center gap-2 px-3 py-2 hover:bg-red-50 text-slate-600 hover:text-red-600 rounded-full transition-colors text-sm font-medium" title="Permanently delete selected items">
