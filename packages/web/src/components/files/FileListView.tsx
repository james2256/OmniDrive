...
export function FileListView(props: FileListViewProps) {
  ...
  const listGridClass = showDriveColumn
    ? 'grid-cols-[28px_1fr_70px_65px_70px_24px] sm:grid-cols-[72px_1fr_140px_120px_140px_44px]'
    : 'grid-cols-[28px_1fr_65px_70px_24px] sm:grid-cols-[72px_1fr_120px_140px_44px]';
  ...
  return (
    <div className="w-full overflow-x-auto">
      {/* Table header */}
      <div className={`grid ${listGridClass} gap-0 bg-surface border-b-2 border-slate-200 px-2 sm:px-4 py-2.5 text-xs font-semibold text-slate-600 uppercase tracking-wide sticky top-0 z-10 group min-w-[320px]`}>
        ...
        return (
          <ItemContextMenu key={key} type="folder" item={folder} actions={actions} isTrashView={isTrashView} isStarred={isStarred}>
            <div
              ...
              className={`grid ${listGridClass} gap-0 items-center px-2 sm:px-4 py-2.5 cursor-pointer transition-colors border-b border-slate-200 group min-w-[320px] ${
                isSelected
