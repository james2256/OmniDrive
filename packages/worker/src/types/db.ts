// ─── D1 Row Types & Mappers ───
//
// `*Row` interfaces mirror the snake_case column layout of the D1 schema
// exactly (see src/db/schema.sql and migrations/). They are the raw shapes
// returned by SQL queries before being converted to camelCase domain types.
//
// `map*Row` functions perform that conversion. They take a generic
// `Record<string, unknown>` (the shape D1's `.bind().all()` yields) and
// return the corresponding domain type from domain.ts.
//
// Anything new here should:
//   - Add the `*Row` interface matching schema.sql 1:1
//   - Add a `map*Row` mapper that returns the domain type
//   - Keep field ordering aligned with the table definition for readability
