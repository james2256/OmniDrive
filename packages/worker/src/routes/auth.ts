...
authRouter.post('/login', zValidator('json', loginSchema, zodErrorHook), async (c) => {
  ...
  const sessionData: SessionData = {
    ...
    role: user.is_super_admin ? 'super_admin' : 'member',
    createdAt: now,
  };
  const sessionId = generateId();
...
authRouter.get('/google', authGuard, async (c) => {
  ...
  if (!env.GOOGLE_CLIENT_ID || !env.GOOGLE_CLIENT_SECRET) {
    throw new AppError(
      400,
      'Google OAuth is not configured. Please login with username and password.',
    );
  }
...
authRouter.get('/callback', async (c) => {
  ...
  if (!targetUserId) {
    throw new AppError(400, 'OAuth session expired — please reconnect your Google account.');
  }
...
authRouter.post(
  '/change-password',
  authGuard,
  zValidator('json', changePasswordSchema, zodErrorHook),
  async (c) => {
