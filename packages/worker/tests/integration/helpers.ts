...
export const TABLES = [
  ...
  `CREATE TABLE IF NOT EXISTS workspaces (id TEXT PRIMARY KEY, name TEXT NOT NULL, owner_id TEXT NOT NULL REFERENCES users(id), created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')), sync_ttl_minutes INTEGER NOT NULL DEFAULT 5, used_bytes INTEGER NOT NULL DEFAULT 0)`,
  ...
  `CREATE TABLE IF NOT EXISTS file_storage_stats (user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE, mime_type TEXT NOT NULL, total_size INTEGER NOT NULL DEFAULT 0, PRIMARY KEY (user_id, mime_type))`,
  ...
  `CREATE TABLE IF NOT EXISTS automation_logs (id TEXT PRIMARY KEY, rule_id TEXT NOT NULL REFERENCES automation_rules(id) ON DELETE CASCADE, status TEXT NOT NULL, details TEXT, executed_at TEXT NOT NULL DEFAULT (datetime('now')))`,
  `CREATE TABLE IF NOT EXISTS s3_multipart_uploads (upload_id TEXT PRIMARY KEY, user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE, workspace_id TEXT NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE, key TEXT NOT NULL, drive_account_id TEXT NOT NULL REFERENCES drive_accounts(id) ON DELETE CASCADE, temp_folder_id TEXT NOT NULL, created_at TEXT NOT NULL DEFAULT (datetime('now')), content_type TEXT)`,
  ...
  `CREATE TABLE IF NOT EXISTS sync_state (drive_account_id TEXT PRIMARY KEY REFERENCES drive_accounts(id) ON DELETE CASCADE, change_token TEXT, next_page_token TEXT, last_synced_at TEXT, status TEXT DEFAULT 'idle', error_message TEXT, locked_at TEXT)`,
