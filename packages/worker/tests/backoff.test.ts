...
describe('isRetryable', () => {
  ...
  it('403 rateLimitExceeded → true', () =>
    expect(isRetryable(403, 'rateLimitExceeded')).toBe(true));
  it('403 dailyLimitExceeded → false', () =>
    expect(isRetryable(403, 'dailyLimitExceeded')).toBe(false));
  it('401 → false (token refresh handles this, not backoff)', () =>
    expect(isRetryable(401, null)).toBe(false));
...
describe('parseDriveError', () => {
  ...
  it('returns fallback message when response.text() fails', async () => {
    ...
    Object.defineProperty(response, 'text', {
      value: () => Promise.reject(new Error('stream consumed')),
    });
    const result = await parseDriveError(response);
...
describe('withBackoff', () => {
  ...
  it('retries on 429 then succeeds', async () => {
    ...
    const promise = withBackoff(fn).then((r) => {
      result = r;
    });
  ...
  it('does not retry on 403 dailyLimitExceeded (throws immediately)', async () => {
    const fn = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          error: { message: 'Daily limit exceeded', errors: [{ reason: 'dailyLimitExceeded' }] },
        }),
        { status: 403 },
      ),
    );
  ...
  it('respects maxRetries — throws after maxRetries+1 attempts', async () => {
    ...
    let error: unknown;
    const promise = withBackoff(fn, { maxRetries: 2 }).catch((e) => {
      error = e;
    });
